...
namespace DirectXTK {
	...
	inline DirectXTK::DescriptorHeap CreateDescriptorHeap(
		ID3D12Device* device, int count
	)
	{ return std::make_unique<DirectX::DescriptorHeap>(device, count); }

	inline DirectX::DescriptorHeap* CreateDescriptorHeap(
		ID3D12Device* device, size_t count
	)
	{ return new DirectX::DescriptorHeap(device, count); }
	...
	inline Sprite CreateSpriteSRV(
		...
		const DirectX::DescriptorHeap* descripterHeap, const size_t index
	)
	{
		Sprite sprite;
	...
	{ return CreateSpriteSRV(device, szFileName, resourceUpload, descripterHeap.get(), index); }
	...
	{ return CreateTextureSRV(device, szFileName, resourceUpload, descripterHeap.get(), index); }
	...
	{ return CreateDefaultSpriteSRV(device, resourceUpload, descripterHeap.get(), index); }
	...
	{ return CreateDefaultTextureSRV(device, resourceUpload, descripterHeap.get(), index); }
